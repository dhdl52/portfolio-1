...
let win=window,winSct,
sections=document.querySelectorAll(".section"),
gnb=document.querySelectorAll(".gnb li"),sideNav=document.querySelectorAll(".sideNav>li");

const gnbOff=()=>{gnb.forEach((o)=>{o.classList.remove("on");})}//입력함수
const navOff=()=>{sideNav.forEach((x)=>{x.classList.remove("on");})}
//function fn//선언적함수

// //console.log(gnb);//변수의 자료의 형태를 파악한다 그뒤에 적당한 함수를 찾아야한다.
gnb.forEach((el)=>{//forEach:소괄호안에 콜백함수를 사용하는애
    //  console.log("나는 el"+el);
    el.addEventListener("click",function(e){
        e.preventDefault();//이벤트속성값쓰기
       // console.log("클릭 el"+el.firstChild);
        document.querySelector(el.firstChild.getAttribute("href"))
...
win.addEventListener("scroll",function(e){
    ...
    winSct>=400?document.querySelector("nav").classList.add("sticky")
    :document.querySelector("nav").classList.remove("sticky")
...
const scrollOn=(sct)=>{
   if(sct>=sections[0].offsetTop-300){//임의의 숫자를 빼주기
    gnbOff();
    navOff();
